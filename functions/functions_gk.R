...
ABCAbas <- function(N, x, hyper, B, g, k) {
  par <- rnorm(N, hyper, 1)
  dist <- sapply(par, function(y) {
    Z <- rgk(length(x), y, B, g, k)
    return(sum(abs(quantile(Z, (0:8) / 8) - quantile(x, (0:8) / 8))))
  })
...
ABCB <- function(N, X, As, g, k) {
  ...
  dist <- numeric(N)
  for (i in 1:N) {
    dist[i] <- sum(sapply(1:n, function(z) {
      K <- rgk(l, As[z], par[i], g, k)
      return(sum(abs(quantile(K, (0:8) / 8) - quantile(X[, z], (0:8) / 8))))
    }))
  }
  return(par[which.min(dist)])
}
