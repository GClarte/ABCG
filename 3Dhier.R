...
# --- Vanilla version of the ABC algorithm
...
# --- Generation of a Pseudo Observations Data Set
a <- 2
...
# ------ Plot for the hyperparameter
f <- ggplot(data = gib_curves30, aes(x = hyperparameter)) +
  theme(
    panel.background = element_rect(fill = "white", colour = "grey"),
    legend.position = "bottom"
  )

# ------ Plot for the parameter mu_1
g <- ggplot(data = gib_curves30, aes(x = mu1)) +
  theme(
    panel.background = element_rect(fill = "white", colour = "grey"),
    legend.position = "bottom"
  )
...
for (i in seq_len(N)) {
  abc_gibbs <- gib_curves30[which(gib_curves30$Group == i & gib_curves30$Method == "ABC-Gibbs"), ]
  abc_vanilla <- gib_curves30[which(gib_curves30$Group == i & gib_curves30$Method == "Simple ABC"), ]
  abc_smc <- gib_curves30[which(gib_curves30$Group == i & gib_curves30$Method == "SMC-ABC"), ]
  # --- Updating f
  f <- f +
    geom_line(data = abc_gibbs, stat = "density", alpha = 0.2) +
    geom_line(data = abc_vanilla, stat = "density", alpha = 0.2) +
    geom_line(data = abc_smc, stat = "density", alpha = 0.2)
  # --- Updating g
  g <- g +
    geom_line(data = abc_gibbs, stat = "density", alpha = .2) +
    geom_line(data = abc_vanilla, stat = "density", alpha = .2) +
    geom_line(data = abc_smc, stat = "density", alpha = .2)
}

# --- Updating f
f <- f + facet_grid(Method ~ ., scales = "free") + xlab(expression(alpha))
f <- f + geom_line(
  data = data.frame(hyperparameter = exact[[2]], Type = rep("Exact", 1000)),
  stat = "density", linetype = "dashed"
) + xlim(c(0, 5)) + scale_colour_viridis_d()
f
...
g <- g + geom_line(
  data = data.frame(mu1 = exact[[1]][, 1], Type = rep("Exact", 1000)),
  stat = "density", linetype = "dashed"
